...
> **状态**：**暂停 / 需重新评审 (2026-06-09)** —— 本计划当前优先级**低于** deep-link-guard 安全修复（见 PR #2），详见下文 §0 "2026-06-09 复核"。
> **创建**：2026-06-08
> **关联**：
...
**复核原因**：同期 deep link guard 安全修复（PR #2）发现，PR1 与该漏洞修复正交。

**5 条短硬可执行项**：

1. **PR1 暂停** —— 本轮不实施。
2. **PR1 不修 deep link 漏洞** —— `/confirm` 和 `/tutorial` server page 接受 `searchParams.text` 但未先走 `buildRouteForInput`，手拼 URL 可绕过首页高风险分流。抽 `shared-classifier.ts` 与该漏洞正交。
3. **"server 跳 /tutorial、client 跳 /confirm" 证据无效**（09:29-31）—— 与 `buildRouteForInput` 实际行为（low/medium 只跳 `/confirm`）不一致。
4. **"两处实现" 应改成 "同一源码进入 client bundle"** —— 准确坏味道是"安全分类器被烤进 client bundle" + "client 兜底链路依赖 domain 风险库"，与 `standards §8.1` 重新对齐。
5. **重新激活前必须重写 §1 / §2** —— 上述问题陈述、行为漂移证据、验收标准均不成立，PR1 plan 整体重做。

**关联**：
- feat/deep-link-guard 安全修复 PR：#2
- `docs/10-feat-deep-link-guard.md` §11：本计划降级决策日志
